...
def main():
    ...
    p.add_argument('--variant', choices=['original', 'rnum_reset'], required=True)
    p.add_argument('--output', required=True)
    p.add_argument('--nvcc', default='/usr/local/cuda-13.1/bin/nvcc')
    a = p.parse_args()
    root = a.root.resolve()
    assert Path(a.output).name == a.output
    source = root/'source' if a.variant == 'original' else root/'variants/rnum_reset'
    record = json.loads((root/'manifest.json' if a.variant == 'original' else source/'VARIANT.json').read_text())
