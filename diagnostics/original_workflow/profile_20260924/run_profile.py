#!/usr/bin/env python3
"""Bounded original-GTS workflow profiling; adapted from original_workflow/run.py."""
import argparse
import fcntl
import hashlib
import json
...
def run(root,gpu,label,case,mode,binary_name,blocking):
    ...
    if mode=='nsys':
        cmd=['nsys','profile','--trace=cuda,nvtx,osrt','--sample=none','--cpuctxsw=none','--cuda-um-cpu-page-faults=true','--cuda-um-gpu-page-faults=true','--export=sqlite','--force-overwrite=false','--output='+str(path/'trace')]+cmd
    env={**os.environ,'CUDA_VISIBLE_DEVICES':gpu,'GTS_DIAG_BLOCKING':str(blocking)}
    (path/'command.json').write_text(json.dumps({'command':cmd,'visible_gpu':gpu,'GTS_DIAG_BLOCKING':blocking},indent=2)+'\n')
    limit=180 if mode in ['memcheck','synccheck','nsys'] else 60
    ...
    observed_flags=[int(x.split(',')[1]) for x in stderr.splitlines() if x.startswith('GTS_FLAGS,')]
    if binary_name=='gts_profile' and (len(observed_flags)!=1 or observed_flags[0]&7 != (4 if blocking else 0)):
        errors.append('Unexpected device scheduling flags: '+str(observed_flags))
    summaries=[x for x in (stderr+'\n'+(path/'stdout.log').read_text()).splitlines() if 'SUMMARY:' in x]
    sanitizer_ok=mode in ['clean','nsys'] or any('ERROR SUMMARY: 0 errors' in x for x in summaries)
    after=snapshot(gpu);(path/'gpu_after.json').write_text(json.dumps(after,indent=2)+'\n')
    record={'label':label,'case':case,'mode':mode,'blocking':blocking,'observed_flags':observed_flags,'binary_name':binary_name,'start':wall_start,'pid':child.pid,'exit_code':rc,'stop_reason':stop_reason,
...
def main():
    ...
    ap.add_argument('--label',required=True);ap.add_argument('--case',choices=['query128','mixed16'],required=True)
    ap.add_argument('--mode',choices=['clean','memcheck','synccheck','nsys'],default='clean')
    ap.add_argument('--binary',default='gts_profile')
    ap.add_argument('--blocking',type=int,choices=[0,1],default=0)
    ...
    ok=run(root,a.gpu,a.label,a.case,a.mode,a.binary,a.blocking)
