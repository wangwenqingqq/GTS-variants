...
inline size_t calcRawDataBytes(int *data_info, float *data_d, char *data_s, int *size_s)
{
	if (data_info[2] != 6)
	{
...
int main(int argc, char **argv)
{
	...
	printf("Loaded Data Info: Dim=%d, Count=%d, Type=%d\n", data_info[0], data_info[1], data_info[2]);
	printf("First data value: %f\n", data_d[0]); // 看看是不是0或者乱码
	...
	cudaDeviceSynchronize(); // 1. 强制 CPU 等待 GPU
	cudaError_t err = cudaGetLastError(); // 2. 抓取错误
	if (err != cudaSuccess) {
		// 3. 打印报错信息并退出
		printf("\n!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
		printf("FATAL ERROR in Index Construction: %s\n", cudaGetErrorString(err));
		printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n\n");
		exit(-1); 
	} else {
		printf("\n[SUCCESS] Index Construction finished without error.\n");
	}
