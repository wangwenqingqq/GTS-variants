...
int main(int argc, char **argv)
{
	...
	else
	{
		...
		const char *update_mode_env = getenv("GTSPP_UPDATE_MODE");
		UPDATE_MODE_KNN = update_mode_env && strcmp(update_mode_env, "knn") == 0;
		if (UPDATE_MODE_KNN)
		{
			search_type = 0;
			k = (int)atoi(argv[4]);
		...
		if (argc > 6) { MAX_IN_SIZE = (int)atoi(argv[6]); } printf("[sweep] MAX_IN_SIZE=%d\n", MAX_IN_SIZE);
	}
	...
	size_t id_map_bytes = static_cast<size_t>(data_info[1]) * 4;
	...
	if (process_type == 0)
	{
		...
		if (file_gt != nullptr) {
			...
			rp_tuner.AutoCalibrateWithoutGroundTruth(
				node_list_h, max_node_num[0], data_h, id_list_h,
				data_info[0], data_info[1], data_info[2], tree_h);
		}
		// Release temporary memory (deferred so GT-free calibration can use data_h)
		delete[] data_h;
		delete[] id_list_h;
		delete[] node_list_h;
