...
int main(int argc, char **argv)
{
	...
	if (process_type != 2)
	{
		file_q = argv[2];
		// ================= [修复开始] =================
        // 1. 去掉 const。argv 本身就是 char**，所以 arg6/arg7 应该是 char*
        char *arg6 = (argc > 6) ? argv[6] : nullptr;
        char *arg7 = (argc > 7) ? argv[7] : nullptr;

        // 2. query_mode 必须保留 const，因为它默认指向字符串常量 "id"
        const char *query_mode = "id";

        // 3. 逻辑判断部分（无需改动逻辑，类型现在匹配了）
        if (arg6 && (strcmp(arg6, "id") == 0 || strcmp(arg6, "vec") == 0 || strcmp(arg6, "vector") == 0))
        {
            query_mode = arg6; // char* 赋值给 const char* (允许，权限收缩)
            file_gt = arg7;    // char* 赋值给 char* (允许，完全匹配)
        }
        else
        {
            file_gt = arg6;    // char* 赋值给 char* (允许，完全匹配)
        }
        // ================= [修复结束] =================
