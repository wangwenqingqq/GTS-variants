...
__managed__ int DIS_CODE = 1000000;
...
__device__ inline double compute_dist_device(const short *data_d, const char *data_s, const int *size_s,
                                             const int *data_info, int idx1, int idx2)
{
	const int dim = data_info[0];
	const int metric = data_info[2];
	double result = 0.0;
	if (metric == 2)
	{ // L2 distance
		for (int j = 0; j < dim; j++)
		{
			double diff = (double)data_d[idx1 * dim + j] - (double)data_d[idx2 * dim + j];
			result += diff * diff;
		}
		result = sqrt(result);
	}
	...
	{ // L1 distance
		for (int j = 0; j < dim; j++)
		{
			result += fabs((double)data_d[idx1 * dim + j] - (double)data_d[idx2 * dim + j]);
		}
	}
	else if (metric == 0)
	{ // Max value
		for (int j = 0; j < dim; j++)
		{
			double diff = fabs((double)data_d[idx1 * dim + j] - (double)data_d[idx2 * dim + j]);
			if (diff > result)
				result = diff;
		}
	}
	...
	{
		double sa1 = 0.0, sa2 = 0.0, sa3 = 0.0;
		for (int j = 0; j < dim; j++)
		{
			double v1 = data_d[idx1 * dim + j];
			double v2 = data_d[idx2 * dim + j];
			sa1 += v1 * v1;
			sa2 += v2 * v2;
			sa3 += v1 * v2;
		}
		...
		if (sa1 * sa2 != 0.0)
		{
			double cosv = sa3 / (sa1 * sa2);
			if (cosv > 1.0)
				cosv = 1.0;
			if (cosv < -1.0)
				cosv = -1.0;
			result = fabs(acos(cosv) * 180.0 / 3.1415926);
		}
	}
	else if (metric == 6)
	{
		...
		if (n != 0 && m != 0)
		{
			...
			for (int j = 1; j <= n; j++)
			{
				for (int k = 1; k <= m; k++)
				{
					int cost = (data_s[idx1 * M + j - 1] == data_s[idx2 * M + k - 1]) ? 0 : 1;
							...
							int *id_list, int start_idx, int *data_info, int *pid_list)
{
	...
	if (split_list[nid] == 1)
	{
		...
		__shared__ int seed[1];
		__shared__ double sh_max[THREAD_NUM];
		__shared__ int sh_idx[THREAD_NUM];

		if (tid == 0)
		{
			seed[0] = id_list[(lid + rid) / 2];
		}
		__syncthreads();

		// Farthest-First Traversal: pick the point farthest from seed as pivot.
		double local_max = -1.0;
		int local_idx = seed[0];
		for (int i = tid + lid; (i >= lid && i <= rid); i += blockDim.x)
		{
			int data_id = id_list[i];
			double dist = compute_dist_device(data_d, data_s, size_s, data_info, data_id, seed[0]);
			if (dist > local_max)
			{
				local_max = dist;
				local_idx = data_id;
		...
		{
			double result = compute_dist_device(data_d, data_s, size_s, data_info, id_list[i], pid[0]);
