							...
							int *id_list, int start_idx, int *data_info, int *pid_list)
{
	...
	if (split_list[nid] == 1)
	{
		...
		for (int i = tid + lid; (i >= lid && i <= rid); i += THREAD_NUM)
		{
			...
			else if (data_info[2] == 8)
			{ // Tanimoto distance
				float dot_ab = 0, sum_a = 0, sum_b = 0;
				for (int j = 0; j < data_info[0]; j++)
				{
					float va = data_d[id_list[i] * data_info[0] + j];
					float vb = data_d[pid[0] * data_info[0] + j];
					dot_ab += va * vb;
					sum_a += va;
					sum_b += vb;
				}
				float denom = sum_a + sum_b - dot_ab;
				result = (denom > 0.0f) ? (1.0f - dot_ab / denom) : 0.0f;
			}
	...
	if (split_list[nid] == 1)
	{
		...
		for (int i = tid; i < TREE_ORDER; i += THREAD_NUM)
		{
			...
			{ // Tanimoto distance
				...
				for (int j = 0; j < data_info[0]; j++)
				{
					float va = data_d[id_list[node_child.lid] * data_info[0] + j];
					float vb = data_d[node_child.pid * data_info[0] + j];
					dot_ab += va * vb;
					sum_a += va;
					sum_b += vb;
				}
