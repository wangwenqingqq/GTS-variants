				  ...
				  int &tree_h, int *&empty_list)
{
	...
	auto computeMaxHeight = [](int order, int object_count) {
		...
		double numerator = log(static_cast<double>(object_count) + 1.0);
		double denominator = log(static_cast<double>(order));
		int h = static_cast<int>(ceil(numerator / denominator)) - 1;
		return h < 1 ? 1 : h;
	};
