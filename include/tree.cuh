...
__managed__ int GTS_PIVOT_POLICY = 0;   // 0: middle, 1: max sample variance, 2: min sample radius, 3: max quantile gap
__managed__ int GTS_PIVOT_SAMPLE = 17;  // sampled objects per candidate when policy=1
...
__device__ __forceinline__ float gtsPointDistance(short *data_d, char *data_s, int *size_s,
												  int aid, int bid, int *data_info)
{
	float result = 0.0f;
	if (data_info[2] == 2)
	{
		...
		{
			float diff = data_d[aid * data_info[0] + j] - data_d[bid * data_info[0] + j];
			result += diff * diff;
		}
		result = sqrtf(result);
	}
	...
	{
		for (int j = 0; j < data_info[0]; j++)
			result += abs(data_d[aid * data_info[0] + j] - data_d[bid * data_info[0] + j]);
	}
	else if (data_info[2] == 0)
	{
		for (int j = 0; j < data_info[0]; j++)
		{
			float temp = abs(data_d[aid * data_info[0] + j] - data_d[bid * data_info[0] + j]);
			if (temp > result)
				result = temp;
		}
	}
	else if (data_info[2] == 5)
	{
		...
		for (int j = 0; j < data_info[0]; j++)
		{
			sa1 += data_d[aid * data_info[0] + j] * data_d[aid * data_info[0] + j];
			sa2 += data_d[bid * data_info[0] + j] * data_d[bid * data_info[0] + j];
			sa3 += data_d[aid * data_info[0] + j] * data_d[bid * data_info[0] + j];
		}
		sa1 = sqrtf(sa1);
		sa2 = sqrtf(sa2);
		if (sa1 * sa2 > 0.0f)
	...
	else if (data_info[2] == 6)
	{
		...
		else
		{
			...
			for (int j = 1; j <= n; j++)
			{
				for (int k = 1; k <= m; k++)
				{
					int cost = (data_s[aid * M + j - 1] == data_s[bid * M + k - 1]) ? 0 : 1;
...
__device__ __forceinline__ int choosePivotId(short *data_d, char *data_s, int *size_s, int *id_list,
											 int lid, int rid, int *data_info)
{
	int size = rid - lid + 1;
	int mid_pos = (lid + rid) / 2;
	if ((GTS_PIVOT_POLICY != 1 && GTS_PIVOT_POLICY != 2 && GTS_PIVOT_POLICY != 3) || size < 8)
		return id_list[mid_pos];

	const int cand_count = 5;
	int sample_count = GTS_PIVOT_SAMPLE;
	if (sample_count < 3)
		sample_count = 3;
	...
	int best_pos = mid_pos;
	for (int c = 0; c < cand_count; c++)
	{
		int cand_pos = lid + (int)(((long long)c * (size - 1)) / (cand_count - 1));
		int cand_id = id_list[cand_pos];
		float sum = 0.0f;
		float sum2 = 0.0f;
		float max_d = 0.0f;
		float sampled_dists[64];
		for (int s = 0; s < sample_count; s++)
		{
			int sample_pos = lid + (int)(((long long)s * (size - 1)) / (sample_count - 1));
			int sample_id = id_list[sample_pos];
			float d = gtsPointDistance(data_d, data_s, size_s, sample_id, cand_id, data_info);
			sampled_dists[s] = d;
			sum += d;
			sum2 += d * d;
			if (d > max_d)
		...
		float var = sum2 / sample_count - mean * mean;
		float score = (GTS_PIVOT_POLICY == 1) ? var : -max_d;
		if (GTS_PIVOT_POLICY == 3)
		{
			for (int a = 1; a < sample_count; a++)
			{
				float key = sampled_dists[a];
				int b = a - 1;
				while (b >= 0 && sampled_dists[b] > key)
				{
					sampled_dists[b + 1] = sampled_dists[b];
					b--;
				}
				sampled_dists[b + 1] = key;
			}
			float gap_sum = 0.0f;
			for (int q = 1; q < TREE_ORDER; q++)
			{
				int right = (q * sample_count) / TREE_ORDER;
				if (right <= 0)
					right = 1;
				if (right >= sample_count)
					right = sample_count - 1;
				float gap = sampled_dists[right] - sampled_dists[right - 1];
				if (gap > 0.0f)
					gap_sum += gap;
			}
			score = gap_sum;
							...
							int *id_list, int start_idx, int *data_info, int *pid_list)
{
	...
	if (split_list[nid] == 1)
	{
		...
		if (tid == 0)
		{
			...
			pid[0] = choosePivotId(data_d, data_s, size_s, id_list, lid, rid, data_info);
			pid_list[nid] = pid[0];
		}
		__syncthreads();
				  ...
				  int &tree_h, int *&empty_list)
{
	...
	const char *pivot_policy_env = getenv("GTS_PIVOT_POLICY");
	if (pivot_policy_env != nullptr)
		GTS_PIVOT_POLICY = atoi(pivot_policy_env);
	const char *pivot_sample_env = getenv("GTS_PIVOT_SAMPLE");
	if (pivot_sample_env != nullptr)
		GTS_PIVOT_SAMPLE = atoi(pivot_sample_env);
	if (GTS_PIVOT_SAMPLE < 3)
		GTS_PIVOT_SAMPLE = 3;
	printf("[PivotPolicy] policy=%d (0=middle, 1=sample_variance, 2=sample_min_radius, 3=sample_quantile_gap), sample=%d\n",
		   GTS_PIVOT_POLICY, GTS_PIVOT_SAMPLE);
