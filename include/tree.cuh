...
#pragma once
#ifndef COMPUTE_DTW_DEFINED
#define COMPUTE_DTW_DEFINED
#define DTW_MAX_LEN 512
__device__ inline float compute_dtw(const float* a, const float* b, int len) {
    float prev[DTW_MAX_LEN], curr[DTW_MAX_LEN];
    prev[0] = fabsf(a[0] - b[0]);
    for (int j = 1; j < len; j++)
        prev[j] = prev[j-1] + fabsf(a[0] - b[j]);
    for (int i = 1; i < len; i++) {
        curr[0] = prev[0] + fabsf(a[i] - b[0]);
        for (int j = 1; j < len; j++) {
            float cost = fabsf(a[i] - b[j]);
            curr[j] = cost + fminf(fminf(prev[j], curr[j-1]), prev[j-1]);
        }
        for (int j = 0; j < len; j++) prev[j] = curr[j];
    }
    return prev[len-1];
}
#endif
							...
							int *id_list, int start_idx, int *data_info, int *pid_list)
{
	...
	if (split_list[nid] == 1)
	{
		...
		for (int i = tid + lid; (i >= lid && i <= rid); i += THREAD_NUM)
		{
			...
			else if (data_info[2] == 7)
			{ // DTW distance
				int len = data_info[0];
				result = (double)compute_dtw(data_d + id_list[i] * len, data_d + pid[0] * len, len);
			}
						  ...
						  int *pid_list, short *data_d, int *id_list, int *data_info, char *data_s, int *size_s)
{
	...
	if (split_list[nid] == 1)
	{
		...
		for (int i = tid; i < TREE_ORDER; i += THREAD_NUM)
		{
			...
			{ // DTW distance
				int len = data_info[0];
				result = (double)compute_dtw(data_d + id_list[node_child.lid] * len, data_d + node_child.pid * len, len);
			}
...
// Fix unsplit bottom-level nodes: mark as leaf
__global__ void forceLeafUnsplit(TN *node_list, int *split_list, int *empty_list, int max_node_num_val)
{
	int idx = blockDim.x * blockIdx.x + threadIdx.x;
	for (int i = idx; i < max_node_num_val; i += blockDim.x * gridDim.x) {
		if (split_list[i] == 1 && empty_list[i] == 0) {
			TN n = node_list[i];
			n.is_leaf = 1;
			node_list[i] = n;
			split_list[i] = 0;
...
{
	printf("Index construction... MAX_H will be computed\n"); fflush(stdout);
	...
	auto computeMaxHeight = [](int order, int object_count) {
		...
		// Ensure tree is deep enough so leaf size <= MAX_SIZE
		double h_for_leaves = ceil(log(static_cast<double>(object_count) / MAX_SIZE) / log(static_cast<double>(order))) + 1;
		int h = static_cast<int>(h_for_leaves);
		return h < 1 ? 1 : h;
	};
	...
	// Force unsplit nodes to leaf (when MAX_H limits depth, bottom nodes may be is_leaf=0)
	forceLeafUnsplit<<<(max_node_num[0] - 1) / THREAD_NUM + 1, THREAD_NUM>>>(
		node_list, split_list, empty_list, max_node_num[0]);
	cudaDeviceSynchronize();

	tree_h = cur_level + 1;
	cudaError_t fe1 = cudaFree(dis_list);
	cudaError_t fe2 = cudaFree(split_list);
	cudaError_t fe3 = cudaFree(split_num);
	cudaError_t fe4 = cudaFree(pid_list);
}