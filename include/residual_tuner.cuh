...
private:
    ...
    static float compute_dist_cpu(const short* data, int idx1, int idx2, int dim, int metric_type) {
        ...
        } else if (metric_type == 7) {  // DTW
            float prev[512], curr[512];
            prev[0] = fabsf((float)data[idx1 * dim] - (float)data[idx2 * dim]);
            for (int j = 1; j < dim; j++)
                prev[j] = prev[j-1] + fabsf((float)data[idx1 * dim] - (float)data[idx2 * dim + j]);
            for (int i = 1; i < dim; i++) {
                curr[0] = prev[0] + fabsf((float)data[idx1 * dim + i] - (float)data[idx2 * dim]);
                for (int j = 1; j < dim; j++) {
                    float cost = fabsf((float)data[idx1 * dim + i] - (float)data[idx2 * dim + j]);
                    curr[j] = cost + fminf(fminf(prev[j], curr[j-1]), prev[j-1]);
                }
                for (int j = 0; j < dim; j++) prev[j] = curr[j];
            }
            return prev[dim-1];
        } else {  // Default: L2
            for (int j = 0; j < dim; j++) {
...
public:
    ...
    // ================================================================
    // [GT-Free Calibration] Estimate gamma from distance distribution
    // When ground truth is not available, estimate gamma per level
    // from the data's distance distribution at leaf nodes.
    // ================================================================
    void AutoCalibrateWithoutGroundTruth(
        TN* node_list, int num_nodes, short* data_h, int* id_list_h,
        int dim, int n, int metric_type, int tree_h)
    {
        printf("\n[GT-Free Calibration] Estimating gamma from distance distribution\n");
        printf("========================================\n");
        tree_h_ = tree_h;
        ...
        for (int i = 0; i < NUM_QUERIES; i++) {
            queries.push_back(query_dist(rng));
        }

        // For each level, collect safe_gamma = d_min / LB_tri ratios
        int level_start = 1;
        for (int level = 1; level < tree_h; level++) {
            int nnum = 1;
            for (int i = 0; i < level; i++) nnum *= TREE_ORDER;

            if (level < 3) {
                // Upper levels: always gamma = 1.0
                gamma_[level] = 1.0f;
                level_start += nnum;
                continue;
            }

            std::vector<float> gamma_ratios;
            int node_step = std::max(1, nnum / 500);  // Sample up to 500 nodes
            int query_step = std::max(1, NUM_QUERIES / 100);  // Sample up to 100 queries

            for (int qi = 0; qi < NUM_QUERIES; qi += query_step) {
                int q = queries[qi];
                ...
                for (int ni = 0; ni < nnum; ni += node_step) {
                    ...
                    // Compute d(q, pivot)
                    float d_qp = compute_dist_cpu(data_h, q, node.pid, dim, metric_type);

                    // Triangle inequality lower bound
                    float dis_lb = fmaxf(0.0f, d_qp - node.min_dis);

                    if (dis_lb < 1e-8f) continue;  // Skip trivial bounds

                    // Estimate d_min: for leaf nodes scan all data;
                    // for internal nodes sample up to 20 random points from subtree
                    float d_min = 1e30f;
                    int scan_count = node.size;
                    const int MAX_SAMPLE = 100;
                    if (scan_count > MAX_SAMPLE) {
                        // Sample MAX_SAMPLE random points from the subtree
                        std::mt19937 sample_rng(qi * 1000 + ni);
                        for (int si = 0; si < MAX_SAMPLE; si++) {
                            int offset = sample_rng() % node.size;
                            int data_id = id_list_h[node.lid + offset];
                            float d = compute_dist_cpu(data_h, q, data_id, dim, metric_type);
                            d_min = fminf(d_min, d);
                        }
                    } else {
                        for (int di = 0; di < node.size; di++) {
                            int data_id = id_list_h[node.lid + di];
                            float d = compute_dist_cpu(data_h, q, data_id, dim, metric_type);
                            d_min = fminf(d_min, d);
                        }
                    }

                    if (d_min > 1e29f || d_min < 1e-8f) continue;

                    // safe_gamma = d_min / LB_tri
                    float safe_gamma = d_min / dis_lb;

                    if (safe_gamma >= 1.0f && safe_gamma < 100.0f) {
                        gamma_ratios.push_back(safe_gamma);
                    }
                }
            }

            float gamma_l = 1.0f;
            if (gamma_ratios.size() >= 10) {
                std::sort(gamma_ratios.begin(), gamma_ratios.end());

                // Use 5th percentile as conservative estimate
                int p5_idx = std::max(0, (int)gamma_ratios.size() / 20);
                float gamma_p5 = gamma_ratios[p5_idx];

                // Apply safety margin (same 75% as GT-based calibration)
                gamma_l = 1.0f + (gamma_p5 - 1.0f) * 0.75f;
                gamma_l = fmaxf(1.0f, fminf(gamma_l, 3.0f));  // Clamp to [1, 3]

                printf("  Level %d: samples=%zu, P5=%.3f, P50=%.3f, gamma=%.4f\n",
                       level, gamma_ratios.size(),
                       gamma_p5,
                       gamma_ratios[gamma_ratios.size() / 2],
                       gamma_l);
            } else {
                printf("  Level %d: insufficient samples (%zu), gamma=1.0\n",
                       level, gamma_ratios.size());
            }

            gamma_[level] = gamma_l;
            level_start += nnum;
        }

        printf("========================================\n");
        printf("[GT-Free Calibration] Complete.\n");
        uploadToGPU();
        PrintStatus();
    }
