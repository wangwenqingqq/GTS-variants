	...
	for (int i = tid; i < node_num; i += blockDim.x)
	{
		...
		if ((query_node_list[query_id * max_node_num[0] + nid_parent] == 1) && (empty_list[nid] == 0))
		{
			...
			{ // L2 distance
				...
				{
					{ float _d = data_d[node.pid * data_info[0] + j] - data_d[qid_list[query_id] * data_info[0] + j]; dis_q += _d * _d; }
				}
				dis_q = sqrtf(dis_q);
			}
	...
	if (bid < search_num[0])
	{
		...
		for (int i = tid; i < node[0].size && node[0].is_leaf == 1; i += blockDim.x)
		{
			...
			{ // L2 distance
				for (int j = 0; j < data_info[0]; j++)
				{
					{ float _d = data_d[data_id * data_info[0] + j] - data_d[qid * data_info[0] + j]; result += _d * _d; }
				}
				result = sqrtf(result);
			}
			else if (data_info[2] == 1)
