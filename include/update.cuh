									 ...
									 int *search_num, char *data_s, int *size_s, int *is_delete)
{
	...
	if (bid < search_num[0])
	{
		...
		// All-include optimization: skip distance computation. Preserve the
		// range-query contract used by the normal path: an active query object
		// stored in the index is a valid distance-zero result. The archived
		// branch excluded it only when a leaf was classified all-include, which
		// made results depend on the traversal optimization.
