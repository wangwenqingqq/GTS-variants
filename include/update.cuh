...
__global__ void getNewData(float *data_d, float *data_d_temp, char *data_s, char *data_s_temp, int *size_s, int *size_s_temp,
...
__global__ void leafProcessRnnUpdate(int *query_lnode, TN *node_list, int *id_list, int *query_qid, float *data_d,
...
void searchIndexRnnUpdate(float *data_d, TN *node_list, int *id_list, int *max_node_num, int *qid_list,
...
void updateIndexRnn(float *&data_d, TN *&node_list, int *&id_list, int *&max_node_num, int *&qid_list, int qnum, float r, int &tree_h,
	...
	for (int i = 0; i < update_num; i++)
	{
		if (update_list[i].update_flag == 0)
		{
			...
			if (in_size == MAX_IN_SIZE)
			{
				...
				if (data_info[2] != 6)
				{
					CHECK(cudaMalloc((void **)&data_d_temp, data_info[1] * data_info[0] * sizeof(float)));
					CHECK(cudaMemcpy(data_d_temp, data_d, data_info[1] * data_info[0] * sizeof(float), cudaMemcpyDeviceToDevice));
					cudaFree(data_d);
					...
					CHECK(cudaMallocManaged((void **)&data_d, data_info[1] * data_info[0] * sizeof(float)));
				}
				else
				{
