					...
					char *&data_s, int *&size_s, FILE *fcost, float &time_update_s, float &time_update_u, int &count_update_s, int &count_update_u)
{
	...
	CHECK(cudaMallocManaged((void **)&obj_r.dis_q, (MAX_IN_SIZE + OVERFLOW_POOL_CAP) * sizeof(float)));
	CHECK(cudaMallocManaged((void **)&obj_r.res_id_q, (MAX_IN_SIZE + OVERFLOW_POOL_CAP) * sizeof(int)));
	CHECK(cudaMemset(is_delete, 0, data_info[1] * sizeof(int)));
	in_size = 0;
	initOverflowPool(max_node_num[0]);
	cudaDeviceSynchronize();
	...
	if (initErr != cudaSuccess) {
		...
		printf("initLeafOverflow OK, max_node_num=%d, pool_cap=%d\n", 
		       max_node_num[0], OVERFLOW_POOL_CAP);
	}
	...
	initOverflowPool(max_node_num[0]);
		...
		else
		{
			...
			// v4: Scan buffer + ONLY visited leaves' overflow entries
			{
				static int *merged_scan_list = nullptr;
				static int merged_cap = 0;
				int overflow_for_visited = 0;

				if (overflow_pool_size > 0) {
					// Copy visited leaf info from GPU to CPU
					int h_search_num = 0;
					cudaMemcpy(&h_search_num, search_num, sizeof(int), cudaMemcpyDeviceToHost);
					int *h_query_lnode = nullptr;
					if (h_search_num > 0) {
						h_query_lnode = (int*)malloc(h_search_num * sizeof(int));
						cudaMemcpy(h_query_lnode, query_lnode, h_search_num * sizeof(int), cudaMemcpyDeviceToHost);
					}

					// Walk linked lists for visited leaves only
					for (int vi = 0; vi < h_search_num; vi++) {
						int lid = h_query_lnode[vi];
						if (lid >= 0 && lid < overflow_max_nodes) {
							int idx = overflow_leaf_head[lid];
							while (idx >= 0 && idx < overflow_pool_size) {
								overflow_for_visited++;
								idx = overflow_pool_next[idx];
							}
						}
					}

					int total_scan = in_size + overflow_for_visited;
					if (total_scan > 0) {
						if (merged_cap < total_scan) {
							if (merged_scan_list) cudaFree(merged_scan_list);
							CHECK(cudaMallocManaged((void **)&merged_scan_list, (total_scan + 64) * sizeof(int)));
							merged_cap = total_scan + 64;
						}
						for (int mi = 0; mi < in_size; mi++)
							merged_scan_list[mi] = insert_list[mi];
						// Copy ONLY visited leaves' overflow entries
						int offset = in_size;
						for (int vi = 0; vi < h_search_num; vi++) {
							int lid = h_query_lnode[vi];
							if (lid >= 0 && lid < overflow_max_nodes) {
								int idx = overflow_leaf_head[lid];
								while (idx >= 0 && idx < overflow_pool_size) {
									merged_scan_list[offset++] = overflow_pool_data[idx];
									idx = overflow_pool_next[idx];
								}
							}
						}
						cudaDeviceSynchronize(); cudaGetLastError();
						searchNaiveRnn(data_info, obj_r, data_d, data_s, size_s, qid_list[0], offset, r, merged_scan_list, rnum);
					}
					if (h_query_lnode) free(h_query_lnode);
				} else if (in_size > 0) {
