...
__constant__ float input_scale[3] = {0.03873674, 0.01071760, 0.01090269};

// Layer 1 Weights (8x3)
__constant__ float w1[8][3] = {
    {-0.212416, -0.063757, -0.217580},
    {-0.169712, -0.483579, -0.375733},
    {-0.039114, 0.163231, -0.254666},
    {0.596426, 1.097545, 1.053107},
    {0.158677, -0.467438, -0.462453},
    {0.352964, 0.668575, 0.341998},
    {-0.432764, -0.371637, 0.008565},
    {1.323832, 0.582798, 0.608248},
};

// Layer 1 Bias
__constant__ float b1[8] = {
    -0.215550, -0.352778, -0.555402, 0.234474, 0.247458, 1.361022, 0.276864, 1.014212
};

// Layer 2 Weights (1x8)
__constant__ float w2[8] = {
    0.234635, -0.089994, -0.098145, 1.110516, 0.025568, 1.346830, -0.033962, 1.524361
};

// Layer 2 Bias
__constant__ float b2 = 0.223180;
// [新增] 设备端 MLP 推理
__device__ float mlp_predict(float lb, float r, float node_min) {
    // 1. 归一化 (使用训练时计算的 scale)
    float in[3];
    in[0] = lb * input_scale[0];
    in[1] = r * input_scale[1];
    in[2] = node_min * input_scale[2];

    // 2. 第一层 FC + ReLU (隐藏层 8 神经元)
    float h[8];
    for (int i = 0; i < 8; ++i) {
        float sum = 0.0f;
        // 手写矩阵乘法，利用寄存器加速
							   ...
							   int offset_p, int offset_up_p, int offset_n, int qs, int qs_up)
{
	...
	for (int i = tid; i < nnum_l * qnum_l; i += total_num)
	{
		...
		if (p_list_k[offset_up_p + ofst_up + nid_parent_p * qnum_up + qid_p_up] == 1 && (empty_list[nid] == 0))
		{
			...
			// [插入代码 START] =========== 数据采集 =============
			// 这是一个原子操作，可能会稍微拖慢速度，但为了收集数据是必须的
			// if (debug_log_count < MAX_LOGS) {
			// 	int idx = atomicAdd(&debug_log_count, 1);
			// 	if (idx < MAX_LOGS) {
			// 		debug_logs[idx].feature_lb = dis_lb;       // 特征1
			// 		debug_logs[idx].feature_r = disk[qid];     // 特征2
			// 		debug_logs[idx].feature_node = node.min_dis; // 特征3
			// 		debug_logs[idx].node_id = nid;
			// 		debug_logs[idx].query_id = qid;
			// 	}
			// }
			// [插入代码 END] ===================================
// [修改 START] ============ AI 剪枝 =============
            ...
            // 调用 MLP 预测保留概率
            float prob = mlp_predict(dis_lb, disk[qid], node.min_dis);

            // 决策逻辑：
            // 1. AI 认为概率 > 0.5 (可调阈值)
            // 2. 且 必须满足原始三角不等式 (Safety Check，保证不发生离谱错误)
            if (prob > 0.5 && dis_lb <= disk[qid]) 
            {
					  ...
					  int qnum, int k, int tree_h, int *data_info, int *empty_list, char *data_s, int *size_s)
{
	...
	while (!st.empty())
	{
		...
		{ // Processing node.
			...
			if (update_disk == false && (pnum_level < k || cur_level <= 1))
			{
	...
	cudaStatus = cudaGetLastError();
	if (cudaStatus != cudaSuccess)
		fprintf(stderr, "log sync error: %s\n", cudaGetErrorString(cudaStatus));
	// FILE *fp = fopen("training_data_raw.csv", "w");
	// if (fp == nullptr)
	// {
	// 	fprintf(stderr, "Failed to open training_data_raw.csv for writing.\n");
	// }
	// else
	// {
