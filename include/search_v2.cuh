	...
	for (int i = tid; i < nnum_l * qnum_l; i += total_num)
	{
		...
		if (p_list[offset_up_p + nid_parent_p * qnum_up + qid_p_up] == 1 && (empty_list[nid] == 0))
		{
			...
			{ // Tanimoto distance
				...
				for (int j = 0; j < data_info[0]; j++)
				{
					float va = data_d[node.pid * data_info[0] + j];
					float vb = data_d[qid_list[qid] * data_info[0] + j];
					dot_ab += va * vb;
					sum_a += va;
					sum_b += vb;
				}
				float denom = sum_a + sum_b - dot_ab;
				dis_q = (denom > 0.0f) ? (1.0f - dot_ab / denom) : 0.0f;
			}
	...
	if (did < node.size && node.is_leaf == 1)
	{
		...
		else if (data_info[2] == 5)
		{
			...
			{ // Tanimoto distance
				...
				for (int j = 0; j < data_info[0]; j++)
				{
					float va = data_d[data_id * data_info[0] + j];
					float vb = data_d[qid_list[qid] * data_info[0] + j];
					dot_ab += va * vb;
					sum_a += va;
					sum_b += vb;
	...
	for (int did = tid; did < MAX_SIZE; did += THREAD_NUM)
	{
		...
		if (did < node.size && node.is_leaf == 1)
		{
			...
			{ // Tanimoto distance
				...
				{
					float va = data_d[data_id * data_info[0] + j];
					float vb = data_d[data_id * data_info[0] + j];
					dot_ab += va * vb;
					sum_a += va;
					sum_b += vb;
				}
	...
	for (int i = tid; i < pnum_level_total * qnum_l; i += total_num)
	{
		...
		if (p_list_k[offset_up_p + ofst_up + nid_parent_p * qnum_up + qid_p_up] == 1 && (empty_list[nid] == 0))
		{
			...
			{ // Tanimoto distance
				...
				{
					float va = data_d[node.pid * data_info[0] + j];
					float vb = data_d[node.pid * data_info[0] + j];
					dot_ab += va * vb;
					sum_a += va;
					sum_b += vb;
				}
