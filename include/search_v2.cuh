							   ...
							   double *p_list_k, int offset_p, int *id_list, int cur_level, int *size_list, int nnum_up)
{
	...
	for (int did = tid; did < MAX_SIZE; did += THREAD_NUM)
	{
		...
		int data_id = (did < node.size && node.is_leaf == 1) ? id_list[node.lid + did] : -1;
						  ...
						  int *res_ids, int qnum, int k, int tree_h, int *data_info, int *empty_list, char *data_s, int *size_s)
{
	...
	while (!st.empty())
	{
		...
		if (cur_level < tree_h)
		{
			...
			else
			{
				...
				// [INSTR] Count visited nodes at this level (only when nodeProcessKnn ran)
				if (update_disk) {
					int _ofst = (nnum_l / TREE_ORDER * 3) * qnum_l;
					double _vis = thrust::reduce(thrust::device,
						p_list_k + offset_p + _ofst,
						p_list_k + offset_p + _ofst + (long long)nnum_l * qnum_l,
						0.0);
					if (cur_level >= 1 && cur_level < 20) _lv_total[cur_level] += (long long)_vis;
				}
		}
		else
		{
	...
	// [INSTR] Print per-level visited node summary
	printf("[VIS_SUMMARY] Per-level average visited nodes (qnum=%d, k=%d):\n", qnum, k);
	for (int _l = 1; _l < tree_h; _l++)
		printf("[VIS] level=%d avg_visited=%.2f\n", _l, (double)_lv_total[_l] / qnum);
	// Release memory
	cudaFree(p_list_k);
	cudaFree(size_list);
	cudaFree(disk);
