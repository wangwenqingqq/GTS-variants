...
// Get memory info before allocations
cudaMemGetInfo(&free_mem_before, &total_mem_before);

CHECK(cudaMallocManaged((void **)&res, qnum * sizeof(int)));
cudaMemGetInfo(&free_mem_after, &total_mem_after);
total_mem_usage += (free_mem_before - free_mem_after);
peak_mem_usage = max(peak_mem_usage, total_mem_usage);
...
printf("Peak memory used by searchIndexRnnV2: %zu MB\n", peak_mem_usage/1024.0/1024.0);
printf("Total memory used by searchIndexRnnV2: %zu MB\n", total_mem_usage/1024.0/1024.0);
}
...
printf("Peak memory used by searchIndexKnnV2: %zu MB\n", peak_mem_usage / 1024 / 1024);
printf("Total memory used by searchIndexKnnV2: %zu MB\n", total_mem_usage / 1024 / 1024);
}